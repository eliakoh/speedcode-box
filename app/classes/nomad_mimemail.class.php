...
class nomad_mimemail
{
	...
	{
		$this->mail_subject = !empty($subject) ? trim(self::mime_header_encode($subject)) : "No subject";
	}
	...
	static function truncate_utf8($string, $len, $wordsafe = FALSE) {
		$slen = strlen($string);
		if ($len > $slen) $len = $slen;
		$i = $len - 1;
		if ($i >= 0 && ord($string[$i]) >= 0x80) {
			$tmp = 0x80;
			while ($i > 0 && (ord($string[$i]) & 0xC0) == 0x80) {
				$tmp = ($tmp >> 1) | 0x80; $i--;
			}
			if ((ord($string[$i]) & (($tmp >> 1) | 0x80)) != $tmp) $len = $i;
		}
		if ($slen <= $len) return $string;
		return substr($string, 0, $len);
	}
	
	static function mime_header_encode($string, $charset = 'UTF-8', $split = true) {
	  if (preg_match('/[^\x20-\x7E]/', $string)) {
	    ...
	    $output = '';
	    while ($len > 0) {
	      $chunk = self::truncate_utf8($string, $chunk_size);
